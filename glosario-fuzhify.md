# Glosario Fuzhify — Vocabulario fijo del proyecto

Este archivo recoge los nombres canónicos de los componentes, métricas y conceptos del proyecto Fuzhify. Aplicar siempre estos términos para mantener coherencia entre secciones de la memoria, entre la memoria y el artículo, y entre la memoria y el código.

## Nombre del sistema

- **Fuzhify** — nombre propio del sistema. Sin acentos, sin espacios. En cursiva la primera vez en cada sección («*Fuzhify*»), después sin cursiva.
...
| t-norma del mínimo | t-norma del mínimo (Zadeh, Mamdani) | t(a, b) = min(a, b). Es la usada por Fuzhify. |
...
| Defuzzificación | defuzzificación | Conversión de salida difusa a un valor numérico. (Fuzhify **no** defuzzifica: la salida es texto, no número.) |
...
La distinción clave para la defensa: **el umbral es absoluto, no relativo a la distribución del conjunto de datos**. Un mismo nivel («Sorprendentes» = *lift* ≥ 2,0) significa lo mismo en cualquier sensor, métrica o dataset. Esto garantiza comparabilidad entre ejecuciones.
...
|-----------------|------------|
| Resumen / informe / reporte | Usar «resumen» para la salida de Fuzhify. «Informe» se reserva al documento global multi-sensor. |
...
| NLG / NLP | NLG (generación) es subcampo de NLP. Fuzhify hace NLG, no NLP en sentido amplio. |
